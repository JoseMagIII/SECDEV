...
public class MgmtHistory extends javax.swing.JPanel {
    ...
    public MgmtHistory(SQLite sqlite, User thatUser, boolean search) {
        initComponents();
        this.thisUser = thatUser;
        this.sqlite = sqlite;
...
//      LOAD CONTENTS
        ...
        for(int nCtr = 0; nCtr < history.size(); nCtr++){
            ...
            if(history.get(nCtr).getUsername().equals(thisUser.getUsername())){
...
//      LOAD CONTENTS
        ...
        for(int nCtr = 0; nCtr < history.size(); nCtr++){
            Product product = sqlite.getProduct(history.get(nCtr).getName());
                 ...
                 else if(history.get(nCtr).getUsername().equals(thisUser.getUsername())){
                            tableModel.addRow(new Object[]{
                            history.get(nCtr).getUsername(), 
                            history.get(nCtr).getName(), 
                            history.get(nCtr).getStock(), 
...
//          LOAD CONTENTS
            ...
            for(int nCtr = 0; nCtr < history.size(); nCtr++){
                if(searchFld.getText().toLowerCase().contains(history.get(nCtr).getUsername().toLowerCase()) || 
                 ...
                 else if(history.get(nCtr).getUsername().toLowerCase().equals(thisUser.getUsername().toLowerCase())){
                            tableModel.addRow(new Object[]{
                            history.get(nCtr).getUsername(), 
                            history.get(nCtr).getName(), 
                            history.get(nCtr).getStock(), 
